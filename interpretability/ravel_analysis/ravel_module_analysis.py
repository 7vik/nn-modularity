...
class intervention:
    ...
    def config_(self):
        ...
        if self.args.model == "gpt2":
            ...
            tokenizer = transformers.GPT2Tokenizer.from_pretrained(
                config[self.args.model]["tokenizer_name"]
            )
            ...
            model = AutoModelForCausalLM.from_pretrained(
                config[self.args.model][self.args.modeltype],
        ...
        logging.basicConfig(
            filename=config[self.args.model]["log_file"],
            level=logging.INFO,
    ...
    def analysis(self, args, module, samples_, config, tokenizer):
        ...
        os.makedirs(
            f"{config[self.args.model]['data_path']}/{self.args.modeltype}/{self.args.entity}",
            exist_ok=True,
        )
        with open(
            f"{config[self.args.model]['data_path']}/{self.args.modeltype}/{self.args.entity}/prediction_{self.args.type_of_intervention}_layer{self.args.num_layer}_{module}.pkl",
            "wb",
        ) as f:
            pkl.dump(prediction, f)

        with open(
            f"{config[self.args.model]['data_path']}/{self.args.modeltype}/{self.args.entity}/samples_{self.args.type_of_intervention}_layer{self.args.num_layer}_{module}.pkl",
            "wb",
        ) as f:
    ...
    def final_analysis(self, args, config):
        ...
        for module in ["mod1", "mod2", "mod3", "mod4"]:
            with open(
                f"{self.config[self.args.model]['data_path']}/{self.args.modeltype}/{self.args.entity}/prediction_{self.args.type_of_intervention}_layer{self.args.num_layer}_{module}.pkl",
                "rb",
            ) as f:
        ...
        plt.grid(True)
        os.makedirs(
            f"{config[args.model]['plot_path']}/{args.modeltype}/{self.args.entity}",
            exist_ok=True,
        )
        plt.savefig(
            f"{config[args.model]['plot_path']}/{args.modeltype}/{self.args.entity}/{args.type_of_intervention}_layer{args.num_layer}_accuracy.png",
            dpi=300,
        )
        plt.close()
        ...
        filtered_lists = [
            (a, b, c, d)
            for a, b, c, d in zip(
                np.ones(np.array(final_dict["mod1"]).shape) - final_dict["mod1"],
    ...
    def old_graph(self):
        ...
        p3 = plt.bar(
            ...
            label="Module 3",
            width=0.5,
            bottom=np.add(self.list1_filtered, self.list2_filtered),
        )
        p4 = plt.bar(
            x,
            self.list4_filtered,
            ...
            width=0.5,
            bottom=np.add(
                self.list1_filtered, np.add(self.list2_filtered, self.list3_filtered)
            ),
        )
        ...
        plt.title(
            "Spike denotes when a module is turned off the accuracy for sample goes down",
            size=16,
        )
        plt.legend()
        plt.grid(True)
        os.makedirs(
            f"{self.config[self.args.model]['plot_path']}/{self.args.modeltype}/{self.args.entity}/{self.args.type_of_intervention}_layer{self.args.num_layer}_effect.png",
            dpi=300,
        )
        plt.close()
    ...
    def pie_chart(self):
        ...
        for sample_idx in range(len(self.list1_filtered)):
            if (
                self.list1_filtered[sample_idx]
                == self.list2_filtered[sample_idx]
                == self.list3_filtered[sample_idx]
                == self.list4_filtered[sample_idx]
                == 1
            ):
                all_four += 1
            elif (
                ...
                == self.list3_filtered[sample_idx]
                == 1
                or self.list1_filtered[sample_idx]
                == self.list2_filtered[sample_idx]
                == self.list4_filtered[sample_idx]
                == 1
                or self.list1_filtered[sample_idx]
                ...
                == self.list4_filtered[sample_idx]
                == 1
                or self.list2_filtered[sample_idx]
                == self.list3_filtered[sample_idx]
                == self.list4_filtered[sample_idx]
                == 1
            ):
                ...
                self.list1_filtered[sample_idx] == self.list2_filtered[sample_idx] == 1
                or self.list1_filtered[sample_idx]
                == self.list3_filtered[sample_idx]
                == 1
                or self.list1_filtered[sample_idx]
                ...
                == self.list4_filtered[sample_idx]
                == 1
                or self.list3_filtered[sample_idx]
                == self.list4_filtered[sample_idx]
                == 1
            ):
                all_two += 1
            elif (
                self.list1_filtered[sample_idx] == 1
                or self.list2_filtered[sample_idx] == 1
                or self.list3_filtered[sample_idx] == 1
                or self.list4_filtered[sample_idx] == 1
            ):
                all_one += 1
        ...
        for i, autotext in enumerate(autotexts):
            autotext.set_color(
                self.get_contrasting_color(colors[i])
            )  # Adjust percentage text color
        ...
        plt.title(
            "Effect of Modules on Samples", pad=20
        )  # Add padding to title for spacing
        ...
        plt.savefig(
            ...
            bbox_inches="tight",
            format="png",
        )  # High DPI for professional quality
        plt.close()
    ...
    def dataset_prepartion(self):
        ...
        total = 0
        samples = []
        for idx, data_ in enumerate(
            tqdm(
                make_wiki_data_loader(self.tokenizer, batch_size=self.args.batch_size),
                desc="Processing batches",
                total=len(
                    make_wiki_data_loader(
                        self.tokenizer, batch_size=self.args.batch_size
                    )
                ),
            )
        ):
        ...
        os.makedirs(
            f"interpretability/ravel_analysis/data/{self.args.model}/{self.args.modeltype}/{self.args.entity}",
            exist_ok=True,
        )
        with open(
            f"interpretability/ravel_analysis/data/{self.args.model}/{self.args.modeltype}/{self.args.entity}/cropped_dataset_last_token.pkl",
            "wb",
        ) as f:
            pkl.dump(samples, f)
    ...
    def hook(self, index):
        ...
        if self.args.model == "pythia70m" or self.args.model == "pythia1.4b":
            self.hook_ = self.model.gpt_neox.layers[
                self.args.num_layer
...
def visualize(config, args):
    ...
    for folder_path in folder_paths:
        ...
        fig, axes = plt.subplots(
            1, num_images, figsize=(num_images * 4, 4)
        )  # Adjust figsize based on the number of images
        ...
        plt.savefig(
            folder_path + "collage.png", dpi=300, bbox_inches="tight", format="png"
        )
