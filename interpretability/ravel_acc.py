...
with open("interp-gains-gpt2small/data/ravel_data/comfy_continent.json", "rb") as f:
    data = json.load(f)
...
def ful_modular_acc():
    ...
    for sample in tqdm(data):
        input_, target = sample[0], sample[1]
        input_ = input_.to(device)
        target = target.to(device)
        tokens = model.to_tokens(input_)
...
def newer_model_acc():
    ...
    total_tlens = 0
    correct_tlens = 0
    model.load_state_dict(torch.load('interp-gains-gpt2small/data/models/new_wiki_modular_mlp_in_model_epoch_1.pt', map_location=device))
...
def newer_nmodel_acc():

    total_tlens = 0
    correct_tlens = 0
    model.load_state_dict(torch.load('interp-gains-gpt2small/data/models/new_wiki_non_modular_mlp_in_model_epoch_1.pt', map_location=device))
...
if __name__ == "__main__":
    ...
    acc3 = newer_model_acc()
    acc4 = newer_nmodel_acc()
