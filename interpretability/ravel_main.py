...
class config:
    ...
    def tlesn_to_nnsight(self):
        for layer in range(12):
            self.model.h[layer].mlp.c_fc = self.model_mod.blocks[layer].mlp.W_in
            self.model.h[layer].mlp.c_proj = self.model_mod.blocks[layer].mlp.W_out
            
            model_mod_attn = torch.cat((self.model_mod.blocks[layer].attn.W_Q, 
                                        self.model_mod.blocks[layer].attn.W_K, 
                                        self.model_mod.blocks[layer].attn.W_V),
                                        dim=1)
            
            self.model.h[layer].attn.c_attn.weight = torch.nn.Parameter(model_mod_attn.view(768, -1))
            
            self.model.h[layer].attn.c_proj.weight = self.model_mod.blocks[layer].attn.W_O
    

    def forward(self):
        self.model_mod = HookedTransformer.from_pretrained('gpt2-small')
        tokenizer = transformers.GPT2Tokenizer.from_pretrained('gpt2')
        device = torch.device(self.args.device)
        self.model_mod.load_state_dict(torch.load('interp-gains-gpt2small/data/models/new_wiki_modular_mlp_in_model_epoch_1.pt', map_location=device, weights_only=True))
        self.model = AutoModel.from_pretrained("openai-community/gpt2")
        self.tlesn_to_nnsight()
        nnsignt_model_mod = LanguageModel(self.model, tokenizer = tokenizer)



        intervened_token_idx = -8
        intervention_token_length = self.args.token_length

        return intervention_model, intervened_token_idx
