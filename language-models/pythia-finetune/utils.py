...
def get_mlp_parameters(model, component: Literal["in", "out", "both"] = "both"):
    ...
    for name, param in model.named_parameters():
        if match := pattern.search(name):
            ...
            layer_idx = next(
                (int(part) for part in name.split(".") if part.isdigit()), None
            )
...
def prepare_hub_name(args, num_clusters, user):
    ...
    hub_name += f"-{bsgc_string}-{lr_string}-{do_modularity_string}-{data_string}"
